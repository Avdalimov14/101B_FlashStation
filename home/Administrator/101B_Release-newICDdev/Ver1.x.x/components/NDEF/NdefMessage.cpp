...
boolean NdefMessage::addRecord(NdefRecord& record)
{
    ...
    {
        ESP_LOGE("NFC LIBS", "WARNING: Too many records. Increase MAX_NDEF_RECORDS.");
        return false;
    }
}
...
{
	ESP_LOGI("NFC LIBS", "\nNDEF Message %d record", _recordCount);//Serial.print(_recordCount);Serial.print(F(" record"));
//    _recordCount == 1 ? Serial.print(", ") : Serial.print("s, ");
	ESP_LOGI("NFC LIBS", "%d bytes", getEncodedSize());//Serial.println(F(" bytes"));

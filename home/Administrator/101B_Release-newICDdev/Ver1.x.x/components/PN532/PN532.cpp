...
/**************************************************************************/
/*!
    Sets the RF field on/off using RFConfiguration register

    @param  on    true - RF Field On, false - RF Field Off

    @returns 1 if everything executed properly, 0 for an error
*/
/**************************************************************************/
bool PN532::setRfField(bool on)
{
	pn532_packetbuffer[0] = PN532_COMMAND_RFCONFIGURATION;
	pn532_packetbuffer[1] = 0x01;
	pn532_packetbuffer[2] = (uint8_t)on;    // Config item 5 (MaxRetries)

	if (HAL(writeCommand)(pn532_packetbuffer, 3))
		return false; // it is -1 PN532_INVALID_ACK

...
uint32_t timePastSeconds;// = 0;
uint8_t timePastEventNum;// = 0;
...
void updateTimeParams()
{
	...
	if (LOG_LOCAL_LEVEL >= ESP_LOG_DEBUG) {
		...
		ESP_LOGD(TIMERLOG, "(%lds since last reset, %lds since last sleep)\n",now.tv_sec,fromLastSleep);
		rtcLastFromResetInSeconds = now.tv_sec;
	}
	timePastSeconds += now.tv_sec;
	nvsTimeWrite();
}
...
//	xTaskCreatePinnedToCore(checkForTimerISR, "TimerISR", 1024, NULL, 2, NULL, 0);
}
